...
- **Cautionary finding for `/init`** (Davis 2024, non-platonic autopoiesis in Asymptotic Lenia): Lenia gliders' coherence is resolution-dependent. The eval harness should measure outcome stability under grid-resolution + dt perturbations to avoid resolution-fragile claims.
...
- `intent_confidence` 0.75 → 0.85 → 0.90 → **0.88** (slight dip after the literature recheck reframed our novelty — but well past the 0.70 threshold for writing `proposed_eval.yaml`).

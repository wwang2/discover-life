...
- **Continuous-attractor memory probe** (`proto/lenia-memory/`): tested the Hopfield analogy on the zoo. **Positive:** each creature is an attractor with a measurable basin radius — Orbium uni/bi tolerate ~30% Gaussian noise; Synorbium and Phantasma break at 5% (order-of-magnitude variation). **Negative:** Lenia is *not* a Hopfield-style memory — pattern completion from half-zeroed Orbium fails completely; different creatures don't co-attract in a single substrate (only Orbium uni & bi survive in Orbium's params, others die). Multi-memory needs multichannel / Flow-Lenia / trained NCAs. **New diagnostic:** `basin_radius` is orthogonal to our 5-leg vector — a 6th leg worth promoting.
